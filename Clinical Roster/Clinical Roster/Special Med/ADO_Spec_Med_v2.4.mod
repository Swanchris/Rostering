...
sum(j in Weekday) (C2_Leave[j] + C2_Long[j] + C2_Rost[j])  == 13;
...
sum(j in Weekday) (C3_Leave[j] + C3_Late[j]) == 14;
...
sum(j in Weekday) (C6_Late[j] + C6_Leave[j]) == 17;
...
sum(j in Weekday) (C10_Leave[j] + C10_Long[j]) == 5;
