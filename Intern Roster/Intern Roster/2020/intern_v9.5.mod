...
//CPC (j=11)  
forall(i in Intern)
  sum(k in 27..54)x[i][11][k] == 4;

//QUM (j=12)  At least one week for each intern before June
forall(i in Intern)
  sum(k in 1..54)x[i][12][k] == 2;
forall(i in Intern)
...
//AP (j=3) 
forall(i in Intern)
  sum(k in 1..52)y3[i][k] ==1;
forall(i in Intern, k in 1..52)
  3 -(sum(a in 0..2)x[i][3][k + a]) <= M*(1-y3[i][k]);
