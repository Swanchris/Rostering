...
//2 @ a time in QUM, Avoid Pub Hol + Sem Days
...
///Intern Rotation Completion Constraint (everyone must do one of the rotations) 
...
//CPC (j=11)  
forall(i in Intern)
  sum(k in 13..54)x[i][11][k] == 4;

//QUM (j=12)  
forall(i in Intern)
  sum(k in 1..33)x[i][12][k] == 2;
  
//H (j=13)  
forall(i in Intern)
  sum(k in 1..33)x[i][13][k] == 1;
